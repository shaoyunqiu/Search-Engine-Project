...
public class ImageIndexer {
	 ...
	 * 
	 */
	public void indexSpecialFile(String filename, Boolean indexHtml){
		try{
			if(indexHtml && fileNum % 1000==0){
				System.out.println("process "+fileNum);
			}
			else if (fileNum % 10000 == 0){
				System.out.println("remove duplication "+fileNum);
			}
			fileNum++;
	        ...
	        if (rootDirhtml.exists()) {
	            ...
	            for (File file : fileshtml) {
	            	if (file.getName().indexOf(".json") > -1) {
	            		...
	            		indexSpecialFile(filename, false);
	            	}
	            }
	        } else {
	...
	public static void main(String[] args) {
        ...
        System.out.println("remove duplication over");
        indexer.fileNum = 0;
        Enumeration<Integer> values = indexer.map.elements();
        while(values.hasMoreElements()) {
        	Integer id = (Integer)values.nextElement();
        	if(id < 52305){
        		indexer.indexSpecialFile(path1 + id + ".json", true);
        	}
        	else {
        		id = id - 52305;
        		indexer.indexSpecialFile(path2 + id + ".json", true);
        	}
        }
        try {
        	indexer.averageLength /= indexer.indexWriter.numDocs();
			System.out.println("average length = "+indexer.averageLength);
			System.out.println("total "+indexer.indexWriter.numDocs()+" documents");
